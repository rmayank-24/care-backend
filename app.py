...
CLINICAL_ANALYSIS_URL = "http://127.0.0.1:8000/api/v1/clinical-analysis"
ONLINE_RESOURCES_URL = "http://127.0.0.1:8000/api/v1/online-resources"
USAGE_STATS_URL = "http://127.0.0.1:8000/api/v1/usage-stats"
RECENT_QUERIES_URL = "http://127.0.0.1:8000/api/v1/recent-queries"
DEBUG_VECTOR_URL = "http://127.0.0.1:8000/api/v1/debug/vector-store"
DEBUG_RETRIEVAL_URL = "http://127.0.0.1:8000/api/v1/debug/retrieval"
...
def get_usage_stats():
    ...
    try:
        response = requests.get(USAGE_STATS_URL)
...
def get_recent_queries(limit=10):
    ...
    try:
        response = requests.get(f"{RECENT_QUERIES_URL}?limit={limit}")
        response.raise_for_status()
...
def debug_vector_store():
    ...
    try:
        response = requests.get(DEBUG_VECTOR_URL)
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def debug_retrieval(query="chest pain"):
    ...
    try:
        response = requests.post(f"{DEBUG_RETRIEVAL_URL}?query={query}")
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
def get_online_resources(disease_name):
    ...
    try:
        ...
        response = requests.post(ONLINE_RESOURCES_URL, json=payload)
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
# --- EXAMPLE REPORTS ---
example_reports = {
    "Cardiac Case": "Patient is a 58-year-old female with a history of type 2 diabetes and obesity. She presents to the emergency department with acute onset of severe substernal chest pain that began 3 hours ago. The pain is described as crushing, radiates to her jaw and left arm, and is associated with diaphoresis and nausea. On examination, she is tachycardic (HR 110), hypertensive (BP 160/95), and has an S4 gallop. ECG shows ST-segment elevation in leads II, III, and aVF. Initial cardiac enzymes show elevated troponin I at 5.2 ng/mL.",
...
with tab1:
        ...
        if not report_input or len(report_input) < 20:
            ...
            with st.spinner("Analyzing report..."):
                try:
                    ...
                    st.subheader("✅ Success: Structured JSON Analysis")
                    analysis = response_data.get("analysis", {})
                    st.json(analysis)

                    st.subheader("📚 Retrieved Knowledge Base Context")
                    st.markdown("The following documents were retrieved from the vector database and used as context for the LLM.")

                    retrieved_docs = response_data.get("retrieved_context", [])
                    if not retrieved_docs:
                        st.info("No documents were retrieved from the knowledge base for this query.")
                    else:
                        for i, doc in enumerate(retrieved_docs):
                            with st.expander(f"**Document {i+1}:** `{doc.get('metadata', {}).get('disease', 'Unknown Disease')}`"):
                                st.text(doc.get('page_content', 'No content available.'))
                                st.caption(f"Source Metadata: {doc.get('metadata', {})}")
                    
                    # Also get the enhanced analysis for structured display
                    try:
                        enhanced_response = requests.post(CLINICAL_ANALYSIS_URL, json=payload, timeout=120)
    ...
    if 'analysis_result' in st.session_state:
        ...
        # Create an expander for more options
            ...
            if 'enhanced_result' in st.session_state and st.session_state.enhanced_result:
                ...
                if differentials:
                    ...
                    for diff in differentials:
                        with st.expander(f"{diff.get('disease_name', 'Unknown')} (Confidence: {diff.get('confidence_score', 0):.2%})"):
                            st.markdown(f"**Summary:** {diff.get('summary', 'No summary available')}")
                            st.markdown(f"**Relevance:** {diff.get('relevance_explanation', 'No explanation available')}")
            ...
            if 'resources_result' in st.session_state:
                ...
                if resources.get("success") and resources.get("resources"):
                    ...
                    for i, resource in enumerate(resources.get("resources", [])):
                        with st.expander(f"{i+1}. {resource.get('title', 'No Title')}"):
                            st.markdown(f"**URL:** [{resource.get('url', 'No URL')}]({resource.get('url', '#')})")
                            st.markdown(f"**Description:** {resource.get('snippet', 'No description available')}")
        ...
        for query in recent_queries:
            with st.expander(f"Query from {query.get('timestamp', 'Unknown time')}"):
                ...
                if retrieved_docs:
                    ...
                    for i, doc in enumerate(retrieved_docs):
                        st.write(f"{i+1}. **Disease:** {doc.get('disease', 'Unknown')}")
