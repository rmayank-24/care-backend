...
def create_document_text(entry):
    ...
    # This function is copied from your ingest_data.py
    fields_to_include = [
...
def main():
    ...
    print("--- Starting Data Ingestion for Pinecone ---")
    load_dotenv()
    ...
    if PINECONE_INDEX_NAME not in pc.list_indexes().names():
        ...
        print(f"  - Dimension: 1024 (for {EMBEDDING_MODEL})")
    ...
    for i, entry in enumerate(data):
        ...
        # Create metadata: start with all fields from the JSON
        metadata = {k: (v if v is not None else "") for k, v in entry.items()}
        
        # CRITICAL: Add the full text (which we embedded) to the metadata
        # This is what main.py will retrieve as 'page_content'
    ...
    print(f"Successfully added {stats.get('total_vector_count', 'N/A')} vectors to the index.")
